    ...
    if (has_intersection) {
        if(s.objects[sphere_id]->is_mirror) {
            ...
            if (s.objects[sphere_id]->is_transparent) {
                ...
                Vector xi = L + randSdir*s.lumiere->R;
                Vector wi = xi-P;
                wi.normalize();
                ...
                Vector dir_al = randomcos_2(N);
                Ray indirectRay(P+eps*N, dir_al);
...
int main() {
    ...
    Sphere slum(Vector(15, 70, -30), R,Vector (1,1,1));
    ...
    Geometry g1("Beautiful Girl.obj", 22, Vector(0,-21, -35), Vector (1,1,1));
